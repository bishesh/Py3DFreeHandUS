...
if __name__ == "__main__":
    ...
    c.calibrateProbe(init, method='eq_based', method_args={'phantom':'single_wall'}, correctResults=False)
    ...
    # Set US image properties
    q.setDataSourceProperties(fromUSFiles=('acc3_Mevis.dcm',), kineFreq=120, pixel2mmX=sx, pixel2mmY=sy)
    
    # Set path for US DICOM files
    q.setUSFiles(('acc3_Mevis.dcm',)) 
    
    # Set path for markers coordinates file
    q.setKineFiles(('acc3.c3d',))
    ...
    q.extractFeatureFromUSImages(feature='2_points', segmentation='manual', featuresFile='acc3.fea')
    
    # Calculate calibration distance accuray (DA)
    q.calculateProbeCalibrationAccuracy(acc='DA', L=30.)
